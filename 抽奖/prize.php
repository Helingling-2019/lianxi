<?php
   ...
   if($ran>=0 && $ran <=9000){
	  ...
	  if($str==0){
		...
		$str = $str - 1;
		$newsql = "UPDATE `prize` SET `shengyu` =$str  WHERE `prizechild`.`ID` = 2";
		mysqli_query($connect,$newsql);
		$ran = 2;  
	  }
	   
   }else if($ran > 9600){
	   $sql = "select shengyu from prize where kinds = 3";
	   $result = mysqli_query($connect,$sql);
	   while($info = mysqli_fetch_array($result)){
	   		   $str = $info["shengyu"];   
	   }
	   if($str==0){
	   		...
	   		$str = $str - 1;
	   		$newsql = "UPDATE `prize` SET `shengyu` =$str  WHERE `prizechild`.`ID` = 3";
	   		mysqli_query($connect,$newsql);
	   		$ran = 3;  
	   }
   }
